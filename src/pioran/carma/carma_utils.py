...
def CARMA_powerspectrum(f: jax.Array,alpha,beta,sigma) -> jax.Array:
    ...
    P = (sigma**2  * jnp.abs(num)**2 /jnp.abs(den)**2).flatten()
    return P

def CARMA_autocovariance(tau,roots_AR,beta,sigma) -> jax.Array:
