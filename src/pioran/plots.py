...
def plot_prediction(
    ...
    **kwargs,
    ...
    np.savetxt(
        f"{filename}_prediction.txt",
        jnp.vstack([x_pred, y_pred, hi, lo, hi2, lo2]).T,
        header="x_pred,y_pred,hi,lo,hi2,lo2",
    )
...
def plot_residuals(
    ...
    **kwargs,
    ...
    ax[1][1].plot(
        norm.pdf(np.linspace(-max_scale_res, max_scale_res, 100), p[0], p[1]),
        np.linspace(-max_scale_res, max_scale_res, 100),
    )
    ax[1][1].hist(
        scaled_residuals,
        bins="auto",
    ...
    ax[0][1].set_xticks([])
    ax[0][1].hist(
        residuals, bins="auto", orientation="horizontal", density=True, alpha=0.75
    )
    ax[0][1].tick_params(
        axis="both", labelleft=False, right="off", left="off", top="off", bottom="off"
    )

    lag, acvf, l, b = ax[2].acorr(
        scaled_residuals, maxlags=maxlag, color="C2", linewidth=2, usevlines=True
    )
    ...
    for i, sig in enumerate(reversed(sigs)):
        ax[2].fill_between(
            np.linspace(0, maxlag, 100),
            sig / np.sqrt(n),
            -sig / np.sqrt(n),
            color="C2",
            alpha=0.15 * (i + 1),
            label=f"{confidence_intervals[i]}%",
        )
        axins.fill_between(
...
def plot_posterior_predictive_ACF(
    tau,
    acf,
    x,
    y,
    ...
    **kwargs,
    ...
    percentiles = jnp.sort(
        jnp.hstack(
            ((50 - np.array(confidence_bands) / 2, 50 + np.array(confidence_bands) / 2))
        )
    )
    ...
    ccf, taulist, npts = xcor(
        x, y, x, y, tlagmin=0, tlagmax=x[-1] / 2, tunit=np.mean(np.diff(x)) / 2, imode=0
    )

    if save_data:
        np.savetxt(
            f"{filename}_posterior_predictive_ACF.txt",
            jnp.vstack([tau, acf_median, acf_quantiles]),
            header="tau,acf_median,acf_quantiles",
        )
        np.savetxt(
            f"{filename}_ICCF.txt", jnp.vstack([taulist, ccf]), header="taulist,ccf"
        )
...
def plot_posterior_predictive_PSD(
    f,
    posterior_PSD,
    x,
    y,
    yerr,
    filename,
    posterior_PSD_approx=None,
    plot_lombscargle=False,
    save_data=False,
    with_mean=False,
    confidence_bands=[68, 95],
    ylim=None,
    xlabel=r"Frequency $\mathrm{d}^{-1}$",
    f_min_obs=None,
    f_max_obs=None,
    **kwargs,
):
    ...
    if posterior_PSD_approx is not None:
        ...
        PSD_quantiles_approx = jnp.percentile(
            posterior_PSD_approx, q=percentiles, axis=0
        )
    ...
    if save_data:
        np.savetxt(
            f"{filename}_LS_periodogram.txt",
            jnp.vstack((f_LS, LS_periodogram)).T,
            header="f_LS,LS_periodogram",
        )
    np.savetxt(
        f"{filename}_posterior_predictive_PSD.txt",
        jnp.vstack((f, psd_median, PSD_quantiles)).T,
        header=f"f,psd_median,psd_quantiles({percentiles})",
    )
    if posterior_PSD_approx is not None:
        np.savetxt(
            f"{filename}_posterior_predictive_PSD_approx.txt",
            jnp.vstack((f, psd_median_approx, PSD_quantiles_approx)).T,
    ...
    if posterior_PSD_approx is not None:
        ...
        for i, ci in enumerate(reversed(confidence_bands)):
            ax.fill_between(
                f,
                PSD_quantiles_approx[i],
                PSD_quantiles_approx[-(i + 1)],
                color="C5",
                alpha=0.15 * (i + 1),
                label=f"{ci}% approx",
            )
    ...
    if plot_lombscargle:
        ax.loglog(f_LS, LS_periodogram, color="C2", label="Lomb-Scargle", alpha=0.35)

    noise_level = np.median(np.diff(x)) * np.median(yerr**2) * 2
    noise_level_mean = np.mean(np.diff(x)) * np.mean(yerr**2) * 2
    ax.axhline(noise_level_mean, color="k", alpha=0.1, label="mean noise level")
    ax.axhline(noise_level, color="k", ls="--", label="median noise level")

    if f_min_obs is not None:
        ax.axvline(f_min_obs, ls="-.", label=r"$f_\mathrm{min}$")
    if f_max_obs is not None:
        ax.axvline(f_max_obs, ls="-.", label=r"$f_\mathrm{max}$")
...
def violin_plots_psd_approx(res, ratio):
    ...
    ax[1].violinplot(
        [mean_ratio, median_ratio, min_ratio, max_ratio], vert=True, showmeans=True
    )
...
def residuals_quantiles(residuals, ratio, f, f_min, f_max):
    ...
    ax[1].fill_between(f, low_5, high_95, alpha=0.2, label=r"$95\%$")
    ax[1].fill_between(f, low_16, high_84, alpha=0.4, label=r"$68\%$")
    ax[1].plot(f, med, label="Median")

    ax[1].update({"xlabel": "Frequency", "ylabel": "Ratios", "xscale": "log"})
...
def plot_priors_samples(params, names):
    ...
    for i in range(n_samples):
        ax[i].hist(params[i, :], bins=150, density=True, alpha=0.25)
...
def plot_prior_predictive_PSD(
    f, psd_samples, xlim=(None, None), ylim=(None, None), xunit=r"$d^{-1}$"
):
    ...
    """
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.loglog(f, psd_samples, c="k", alpha=0.01)
