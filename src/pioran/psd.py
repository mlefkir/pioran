...
class TwoBendPowerLaw(PowerSpectralDensity):
    r"""Two-bend power-law power spectral density.
    ...
    def __init__(
        self, parameters_values, free_parameters=[False, True, True, True, True, True]
    ):
        assert (
            len(parameters_values) == 6
        ), f"The number of parameters for onebend-powerlaw must be 6, not {len(parameters_values)}"
        assert (
            len(free_parameters) == 6
        ), f"The number of free parameters for onebend-powerlaw must be 6, not {len(free_parameters)}"
        # initialise the parameters and check
        names = ["norm", "index_1", "freq_1", "index_2", "freq_2", "index_3"]
    ...
    def calculate(self, f):
        ...
        index_1, f_1, index_2, f_2, index_3, norm = (
            self.parameters["index_1"].value,
            self.parameters["freq_1"].value,
            self.parameters["index_2"].value,
            self.parameters["freq_2"].value,
            self.parameters["index_3"].value,
            self.parameters["norm"].value,
        )
        P = (
            jnp.power(f / f_1, -index_1)
            * jnp.power(1 + jnp.power(f / f_1, index_2 - index_1), -1)
            * jnp.power(1 + jnp.power(f / f_2, index_3 - index_2), -1)
        )
        return P * norm
