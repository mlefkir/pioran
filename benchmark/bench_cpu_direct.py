...
for i,N in enumerate(n_samples):
	...
	psd = OneBendPowerLaw(p)
	gp = GaussianProcess(psd,t_trunc,y_trunc,yerr_trunc,method='FFT')
	...
	full_mean.append(res[1]/res[0])

data = np.array(full_mean)
data = np.vstack((n_samples,data))
np.savetxt(f'benchmark_cpu_direct.txt',data,header="Benchmark FFT CPU\ntime(s)")
